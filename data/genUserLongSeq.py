...
def main():
    ...
    # 定义输出文件路径，为完整序列
    output_files = {
        "item_id": os.path.join(data_dir, "userId2itemId.txt"),
        "cate_id": os.path.join(data_dir, "userId2cateId.txt"),
        "brand": os.path.join(data_dir, "userId2brand.txt"),
        "u_cate_click_cnt": os.path.join(data_dir, "userId2uCateClickCnt.txt"),
        "u_brand_click_cnt": os.path.join(data_dir, "userId2uBrandClickCnt.txt")
    }
    
    # 使用 defaultdict(list) 存储全量序列
    user_seqs = {
    ...
    # 1. 遍历 train_joined_logs.csv 提取全量点击序列 (clk=1)
    print("正在处理 train_joined_logs.csv 并提取完整长序列（包含交叉特征）...")
