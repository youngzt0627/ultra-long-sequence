...
class DINDataset(Dataset):
    """
    数据集类：将 DataFrame、短序列 和 同类目双索引序列 转换为 Tensor。
    """
    def __init__(self, df, seq_dict, dual_seq_dict, description, encoders, device, max_len=20):
        self.df = df
        self.seq_dict = seq_dict
        self.dual_seq_dict = dual_seq_dict
    ...
    def _prepare_data(self):
        ...
        for name in self.seq_names:
            base_name = name.replace('hist_', '').replace('sim_', '').replace('_seq', '')
            ...
            # 确定数据源与 Key 构造方式
            is_sim = name.startswith('sim_')
            src_dict = self.dual_seq_dict if is_sim else self.seq_dict
            
            uids = self.df['user_id'].astype(str).tolist()
            cids = self.df['cate_id'].astype(str).tolist()
            
            for i in range(len(uids)):
                key = f"{uids[i]}_{cids[i]}" if is_sim else uids[i]
                raw_seq = src_dict.get(name).get(key, [])
...
class DINDataLoaders:
    def __init__(self, data_dir, sim_dir, bsz, device, max_len=20, sample_n=100000):
        ...
        # 3. 加载同类目双索引序列字典 (来自 sim 目录，包含交叉特征序列)
        print("Loading dual-index same-category sequence dictionaries (including cross features)...")
        dual_seq_dict = {
            "sim_item_seq": self._load_seq(os.path.join(sim_dir, 'userId_cateId2itemId.txt')),
            "sim_cate_seq": self._load_seq(os.path.join(sim_dir, 'userId_cateId2cateId.txt')),
            "sim_brand_seq": self._load_seq(os.path.join(sim_dir, 'userId_cateId2brand.txt')),
            "sim_u_cate_click_cnt_seq": self._load_seq(os.path.join(sim_dir, 'userId_cateId2uCateClickCnt.txt')),
            "sim_u_brand_click_cnt_seq": self._load_seq(os.path.join(sim_dir, 'userId_cateId2uBrandClickCnt.txt'))
        }
        ...
        # 增加同类目双索引序列特征描述 (包含交叉特征序列)
        description.append(('sim_item_seq', encoders['item_id'].__len__() + 1, 'seq'))
        description.append(('sim_cate_seq', encoders['cate_id'].__len__() + 1, 'seq'))
        description.append(('sim_brand_seq', encoders['brand'].__len__() + 1, 'seq'))
        description.append(('sim_u_cate_click_cnt_seq', 1, 'seq_ctn'))
        description.append(('sim_u_brand_click_cnt_seq', 1, 'seq_ctn'))
        ...
        train_ds = DINDataset(train_df, seq_dict, dual_seq_dict, description, encoders, device, max_len)
        test_ds = DINDataset(test_df, seq_dict, dual_seq_dict, description, encoders, device, max_len)
...
class SIMModel(nn.Module):
    def __init__(self, description, embed_dim=10, mlp_dims=(128, 64), dropout=0.2):
        ...
        for name, size, t in description:
            if t == 'spr':
                ...
                input_dim += 1
            elif t == 'seq' and name in ('hist_item_seq', 'sim_item_seq'):
                # 兴趣向量维度：item(8) + cate(8) + brand(8) + cross_cate(1) + cross_brand(1) = 26
                input_dim += 3 * embed_dim + 2 
                
        self.hist_attention = TargetMultiHeadAttention(num_units=3 * embed_dim + 2, num_heads=8)
        self.sim_attention = TargetMultiHeadAttention(num_units=3 * embed_dim + 2, num_heads=8)
    ...
    def forward(self, x_dict):
        ...
        hist_interest = self.hist_attention(qi_full, H_hist, H_hist, mask_hist)
        
        # 3. 同类目序列建模 (SIM)
        sim_item_emb = self.item_embed(x_dict['sim_item_seq'])
        sim_cate_emb = self.cate_embed(x_dict['sim_cate_seq'])
        sim_brand_emb = self.brand_embed(x_dict['sim_brand_seq'])
        # 同类目序列向量：嵌入向量序列 + 交叉特征序列
        H_sim = torch.cat([
            sim_item_emb, sim_cate_emb, sim_brand_emb,
            x_dict['sim_u_cate_click_cnt_seq'].unsqueeze(-1),
            x_dict['sim_u_brand_click_cnt_seq'].unsqueeze(-1)
        ], dim=-1)
        mask_sim = (x_dict['sim_item_seq'] > 0).float()
        sim_interest = self.sim_attention(qi_full, H_sim, H_sim, mask_sim)
        ...
        all_feats.append(hist_interest)
        all_feats.append(sim_interest)
...
def train_eval():
    ...
    parser.add_argument('--sim_dir', default='./sim')
    ...
    print(f"Loading data from {args.data_dir} and {args.sim_dir}...")
    loaders = DINDataLoaders(args.data_dir, args.sim_dir, args.bsz, device)
    
    print("Building SIM Model (Short Seq + Same Cate Seq)...")
    model = SIMModel(loaders.description).to(device)
